...
# class transmitter_mapper(gr.hier_block2):
    # def __init__(self, modtype, m, txname, samples_per_symbol=2, excess_bw=0.35):
...
class transmitter_mpsk(gr.hier_block2):
    def __init__(self, m, modname):
        self.modname = modname
        constellation = digital.psk_constellation(m)
        gr.hier_block2.__init__(self, "transmitter_" + modname,
            gr.io_signature(1, 1, gr.sizeof_char),
            gr.io_signature(1, 1, gr.sizeof_gr_complex))
        self.repack = blocks.unpacked_to_packed_bb(1, gr.GR_MSB_FIRST)
        self.mod = digital.generic_mod(constellation, samples_per_symbol=sps, excess_bw=ebw)
        self.connect( self, self.repack, self.mod, self )


class transmitter_mqam(gr.hier_block2):
    def __init__(self, m, modname):
        self.modname = modname
        constellation = digital.qam_constellation(m)
        gr.hier_block2.__init__(self, "transmitter_" + modname,
            gr.io_signature(1, 1, gr.sizeof_char),
            gr.io_signature(1, 1, gr.sizeof_gr_complex))
        self.repack = blocks.unpacked_to_packed_bb(1, gr.GR_MSB_FIRST)
...
class transmitter_bpsk(transmitter_mpsk):
    modname = "BPSK"
    def __init__(self):
        transmitter_mpsk.__init__(self, 2, self.modname)

class transmitter_qpsk(transmitter_mpsk):
    modname = "QPSK"
    def __init__(self):
        transmitter_mpsk.__init__(self, 4, self.modname)

class transmitter_8psk(transmitter_mpsk):
    modname = "8PSK"
    def __init__(self):
        transmitter_mpsk.__init__(self, 8, self.modname)

class transmitter_qam16(transmitter_mqam):
    modname = "QAM16"
    def __init__(self):
        transmitter_mqam.__init__(self, 16, self.modname)

class transmitter_qam64(transmitter_mqam):
    modname = "QAM64"
    def __init__(self):
        transmitter_mqam.__init__(self, 64, self.modname)

# class transmitter_bpsk(gr.hier_block2):
    # modname = "BPSK"
    # def __init__(self):
        # gr.hier_block2.__init__(self, "transmitter_bpsk",
            # gr.io_signature(1, 1, gr.sizeof_char),
            # gr.io_signature(1, 1, gr.sizeof_gr_complex))
        # self.repack = blocks.unpacked_to_packed_bb(1, gr.GR_MSB_FIRST)
        # constellation = digital.constellation_bpsk
        # self.mod = digital.bpsk_mod(samples_per_symbol=sps, excess_bw=ebw)
        # self.connect( self, self.repack, self.mod, self )
